...
# Set up Jinja2 template engine for rendering HTML templates for reading parts that not plain html
...
class DataForm:
    ...
    async def get_vehicle_data(self):
        ...
        # This line is used in FastAPI (and Starlette, which FastAPI is built on) to extract form data sent via a POST request from an HTML form.
...
'''
🧠 Why pass {"request": request}?
Jinja2 inside FastAPI needs request to:
Resolve URL paths properly.
Use template features like {% if %}, {% for %}, or request-related logic.
'''
